...
import DatosEntrenamiento
import joblib #esta es la libería que permite guardar el modelo


# Creacion y entrenamiento del modelo
modelo_bayesiano = GaussianNB()
modelo_bayesiano.fit(DatosEntrenamiento.datos_entrenamiento, DatosEntrenamiento.etiquetas)


#modelo_nuevoFiltrado = joblib.dump('nombre.pkl') donde nombre es el nombre del nuevo modelo que quieras 
# guardar y dump es la función de la librería que permite guardar el modelo, esta es la basepara guardar un modelo
joblib.dump(modelo_bayesiano, 'modBayes_fit.pkl')

# nuevo caso
nueva_pc = np.array([[8, 2, 1, 2, 2, 4]])  # uso

# Predicción
prediccion = modelo_bayesiano.predict(nueva_pc)
categorias = {
0: "Lenovo IdeaPad 3",
...
def IniConso():
    for i in range (100):
        print('Ingrese unos datos chidoris de que le gustaría masomenos: \n')
        val1 = str(input('Ingrese un precio Aproximado: '))
...
'''
print(f"La computadora probablemente es: {categorias[prediccion[0]]}")